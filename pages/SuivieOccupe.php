...
<script>
    <?php if(isset($_POST['annee'])) { ?>
        ...
        const data = {
            labels: labels,
            datasets: [{
                label: 'courbe de suivie nombre d\'habitation loué : ',
                backgroundColor: 'rgb(255, 99, 132)',
                borderColor: 'rgb(255, 99, 132)',
                // data: [0, 10, 5, 2, 20, 30, 45],
                data:[
                <?php for($i=1;$i<=getJourMois($_POST['mois'],$_POST['annee']);$i++) { ?>
                    <?php if(getChambeOccuper($_POST['mois'],$_POST['annee'],$i)=="") {  echo "0,"; ?>
                        <?php } else {  echo getChambeOccuper($_POST['mois'],$_POST['annee'],$i)."," ?>
                    <?php } ?>
                <?php } ?>
                ]
            }]
